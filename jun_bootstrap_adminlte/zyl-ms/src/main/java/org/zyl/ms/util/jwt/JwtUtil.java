 ...
 * @time 2019-05-10 20:32:04
 */
public class JwtUtil {
	public static final long TTL_ONE_YEAR = 365 * 24 * 60 * 60 * 1000;
	 ...
	 * @return token
	 */
	public static String createJWT(long ttlMillis, String jwtKey, SessionUser user) {
		...
		claims.put("password", user.getUserPass());
	 ...
	 * @return
	 */
	public static long getUserId(String token, String jwtKey) {
		// 得到DefaultJwtParser
		Claims claims = Jwts.parser()
				// 设置签名的秘钥 , 和生成的签名的秘钥一模一样
				.setSigningKey(jwtKey)
		...
		if (ValidateUtil.isNumeric(claims.get("id")+"")) {
			return new Long(claims.get("id")+"");
		}
