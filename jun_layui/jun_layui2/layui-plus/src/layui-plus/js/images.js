layui.define(['flow','form','layer','upload','globalConfig'], function(exports){
    var flow = layui.flow,
        form = layui.form,
    ...
    flow.load({
        elem: '#Images', //流加载容器
            ...
            $.get(layui.globalConfig.rootPath + "json/images.json",function(res){
                //模拟插入
                var imgList = [],data = res.data;
                var maxPage = imgNums*page < data.length ? imgNums*page : data.length;
                setTimeout(function(){
                    for(var i=imgNums*(page-1); i<maxPage; i++){
                        imgList.push('<li><img layer-src="'+ layui.cache.base + data[i].src +'" src="'+ layui.cache.base + data[i].thumb +'" alt="'+data[i].alt+'"><div class="operate"><div class="check"><input type="checkbox" name="belle" lay-filter="choose" lay-skin="primary" title="'+data[i].alt+'"></div><i class="layui-icon img_del">&#xe640;</i></div></li>');
                    }
    ...
    upload.render({
        elem: '.uploadNewImg',
        url: layui.globalConfig.rootPath + 'json/userface.json',
        multiple: true,
        before: function(obj){
            //预读本地文件示例，不支持ie8
