layui.define(['form','layer','laydate','table','upload','globalConfig'], function(exports){
    var form = layui.form,
    ...
    var tableIns = table.render({
        elem: '#linkList',
        url : layui.globalConfig.rootPath + 'json/linkList.json',
        page : true,
        cellMinWidth : 95,
        height : "full-104",
        limit : 20,
        ...
        cols : [[
            ...
            {field: 'logo', title: 'LOGO', width:180, align:"center",templet:function(d){
                return '<a href="'+d.websiteUrl+'" target="_blank"><img src="'+ layui.cache.base + d.logo+'" height="26" /></a>';
            }},
    ...
    function addLink(edit){
        var index = layer.open({
            ...
            type : 2,
            area : ["300px","385px"],
            content : layui.globalConfig.distPath + "page/systemSetting/linksAdd.html",
            success : function(layero, index){
                var body = $($(".layui-layer-iframe",parent.document).find("iframe")[0].contentWindow.document.body);
                if(edit){
                    body.find(".linkLogo").css("background","#fff");
                    body.find(".linkLogoImg").attr("src",layui.cache.base + edit.logo);
    ...
    upload.render({
        elem: '.linkLogo',
        url: layui.globalConfig.rootPath + 'json/linkLogo.json',
        method : "get",  //此处是为了演示之用，实际使用中请将此删除，默认用post方式提交
        done: function(res, index, upload){
            var num = parseInt(4*Math.random());  //生成0-4的随机数，随机显示一个头像信息
			console.log(layui.cache.base + res.data[num].src)
            $('.linkLogoImg').attr('src',layui.cache.base + res.data[num].src);
