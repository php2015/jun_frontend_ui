...
if (typeof jQuery === "undefined") {
    ...
    defaultLayoutTemplates = {
           ...
           '<div class="mt-nav-bar {navBarClass}" style="background-color: {backgroundColor};">' +
               '<div class="mt-nav mt-nav-tools-left">' +
                   '<ul  class="nav {nav-tabs}">' +
                       '<li class="mt-move-left"><a><i class="layui-icon" style="font-size: 12px;">&#xe603;</i></i></a></li>' +
                   '</ul>' +
               '</div>' +
               ...
               '<div class="mt-nav mt-nav-tools-right">' +
                   '<ul  class="layui-nav nav {nav-tabs}">' +
                       '<li class="mt-move-right"><a><i class="layui-icon" style="font-size: 12px;">&#xe602;</i></a></li>' +
                       '<li class="layui-nav-item">' +
                           '<a href="javascript:;" class="ss"><i style="font-size:16px;" class="fa fa-bars"></i></a>' +
                           '<dl class="layui-nav-child dropdown-menu dropdown-menu-right">' +
                               '<dd class="mt-show-actived-tab"><a>{showActivedTab}</a></dd>' +
                               '<dd class="divider"></dd>' +
                               '<dd class="mt-close-all-tabs"><a>{closeAllTabs}</a></dd>' +
                               '<dd class="mt-close-other-tabs"><a>{closeOtherTabs}</a></dd>' +
                           '</dl>' +
                       '</li>' +
                   '</ul>' +
               '</div>' +
        ...
        closeBtn : ' <i class="mt-close-tab layui-icon" style="font-size: 10px;">&#x1006;</i>',
            ...
            if(options.draggable){
                handler($el.navPanelList, 'mousedown', '.mt-nav-tab', function (event) {
                    ...
                    $navTabLi.css({'left': event.pageX - abs_x + 'px', 'position': 'absolute', 'z-index': 9999})
                        ...
                        .find('a:first').css({'background' : '#009688'});
    ...
    $.fn.multitabs.defaults = {
        ...
        init : [									//需要在初始加载的tab
			{                                       
                content :'main',                         //标签页的类型，有 main | info | editor
                title : '首页',                          //标题（可选），没有则显示网址
                url : 'pages/home.html'             	//链接，如为外链，强制为info页
            }, 
            {  /** 更多tab。。**/  },                		//依次添加需要的页面
            {  /** 更多tab。。**/  },                		//依次添加需要的页面
		],                                   	  
        ...
        language : {                                //语言配置
            navBar : {
                ...
                showActivedTab : '显示激活“选项卡',          		//下拉菜单的显示激活页面
                closeAllTabs : '关闭所有选项卡',                	//下拉菜单的关闭所有页面
                closeOtherTabs : '关闭其他选项卡',            	//下拉菜单的关闭其他页面
            },
            editorUnsave: {
