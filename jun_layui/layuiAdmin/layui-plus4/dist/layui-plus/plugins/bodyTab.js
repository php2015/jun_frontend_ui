...
layui.define(["element","jquery","globalConfig"],function(exports){
            ...
            if(data[i].children != undefined && data[i].children.length > 0){
                ...
                for(var j=0;j<data[i].children.length;j++){
                    if(data[i].children[j].target == "_blank"){
                        ulHtml += '<dd><a data-url="' + layui.globalConfig.distPath + data[i].children[j].href+'" target="'+data[i].children[j].target+'">';
                    }else{
                        ulHtml += '<dd><a data-url="' + layui.globalConfig.distPath +data[i].children[j].href+'">';
                    }
                ...
                if(data[i].target == "_blank"){
                    ulHtml += '<a data-url="' + layui.globalConfig.distPath + data[i].href+'" target="'+data[i].target+'">';
                }else{
                    ulHtml += '<a data-url="' + layui.globalConfig.distPath + data[i].href+'">';
                }
                if(data[i].icon != undefined && data[i].icon != ''){
                    if(data[i].icon.indexOf("icon-") != -1){
                        ulHtml += '<i class="seraph '+data[i].icon+'" data-icon="'+data[i].icon+'"></i>';
	...
	Tab.prototype.render = function() {
		//显示左侧菜单
		var _this = this;
		$(".navBar ul").html('<li class="layui-nav-item layui-this"><a data-url="' + layui.globalConfig.distPath + 'page/main.html"><i class="layui-icon" data-icon=""></i><cite>后台首页</cite></a></li>').append(_this.navBar(dataStr)).height($(window).height()-210);
