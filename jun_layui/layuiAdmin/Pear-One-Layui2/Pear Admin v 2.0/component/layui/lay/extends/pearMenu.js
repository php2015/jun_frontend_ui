	...
	function createMenu(option){
		...
		// 开 启 同 步 操 作
		    $.each(option.data, function(i, item) {
				...
				if (item.type == 0) {
					// 创 建 目 录 结 构
					content += '<a  href="javascript:;" menu-type="'+item.type+'" menu-id="'+item.id+'" href="javascript:;"><i class="' + item.icon + '"></i><span>' + item.title +
						'</span></a>';
				} else if (item.type == 1) {
					// 创 建 菜 单 结 构
					content += '<a class="site-demo-active" menu-type="'+item.type+'" menu-url="' + item.href + '" menu-id="' + item.id +
						'" menu-title="' + item.title + '" href="javascript:;" href="javascript:;"><i class="' + item.icon +
	...
	function loadchild(obj) {
		...
		if (obj.children != null && obj.children.length > 0) {
			...
			$.each(obj.children, function(i, note) {
				...
				if (note.type == 0) {
					// 创 建 目 录 结 构
	                content += '<a  href="javascript:;" menu-type="'+note.type+'" menu-id="'+note.id+'"><i class="' + note.icon + '"></i><span>' + note.title + '</span></a>';
				} else if (note.type == 1) {
					// 创 建 菜 单 结 构
					content += '<a class="site-demo-active" menu-type="'+note.type+'" menu-url="' + note.href + '" menu-id="' + note.id +'" menu-title="' + note.title + '" menu-icon="' + note.icon + '" href="javascript:;"><i class="' + note.icon +'"></i><span>' + note.title + '</span></a>';
				}
				// 加 载 子 项 目 录
				content += loadchild(note);
				// 结 束 当 前 子 菜 单
