...
function Header(calendar, options) {
		...
		if (buttonStr) {
			$.each(buttonStr.split(' '), function(i) {
				...
				$.each(this.split(','), function(j, buttonName) {
					...
					if (buttonName == 'title') {
					    groupChildren = groupChildren.add($('<select name="year"></select> &nbsp; <select name="month"></select>')); // we always want it to take up height
						isOnlyButtons = false;
					}
					else {
	...
	function updateTitle(text) {
	    var d = text.split(' ');
	    if (el.find('select[name="year"]>option').size() === 0) {
	        for (var i = 2015; i <= new Date().getFullYear()+1 ; i++) {
	            el.find('select[name="year"]').append('<option value="' + i + '">' + i + '年</option>');
	        }
	        var monthDigitCN = ['一', '二', '三', '四', '五', '六', '七', '八', '九', '十', '十一', '十二'];
	        for (var i = 0; i < 12; i++) {
	            el.find('select[name="month"]').append('<option value="' + i + '">' + monthDigitCN[i] + '月</option>');
	        }
	        el.find('select[name="year"]').change(function () {
	            calendar.gotoDate(new Date($(this).val(), el.find('select[name="month"]').val()));
	        });
	        el.find('select[name="month"]').change(function () {
	            calendar.gotoDate(new Date(el.find('select[name="year"]').val(), $(this).val()));
	        });
	    }
        
	    el.find('select[name="year"]>option').filter(function () {
	        return $(this).text() == $.trim(d[0]);
	    }).prop('selected', true);

	    el.find('select[name="month"]>option').filter(function () {
	        return $(this).text() == $.trim(d[1]);
	    }).prop('selected', true);
	}
