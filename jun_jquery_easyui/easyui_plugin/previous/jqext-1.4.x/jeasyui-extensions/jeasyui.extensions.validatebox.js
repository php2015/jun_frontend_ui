...
*
* jQuery EasyUI validatebox Extensions
...
(function ($, undefined) {
    ...
    var defaults = $.fn.validatebox.extensions.defaults = {
        ...
        // 增加 easyui-validatebox 控件的自定义扩展属性；该属性功能类似于 easyui-searchbox 的 prompt 属性，表示该验证输入框的提示文本；
        // String 类型值，默认为 null。
        prompt: null,

        // 增加 easyui-validatebox 控件的自定义扩展属性；该属性表示在当前页面加载完成后，该 easyui-validatebox 控件是否自动获得焦点。
        // Boolean 类型值，默认为 false。
        autoFocus: false,

        // 增加 easyui-validatebox 控件的自定义扩展属性；表示是否在该控件初始化完成后立即进行一次验证；默认为 true。
        autoValidate: true,

        // 增加 easyui-validatebox 控件的自定义扩展属性；表示其初始化时的值。
        value: null,

        // 增加 easyui-validatebox 控件的自定义扩展属性；表示其初始化时的宽度值。
        width: null,

        // 增加 easyui-validatebox 控件的自定义扩展属性；表示该控件在初始化完成后是否设置其为禁用状态(disabled)；默认为 false。
        disabled: false,

        // 增加 easyui-validatebox 控件的自定义扩展属性；表示该控件在初始化完成后是否设置其为只读状态(readonly)；默认为 false。
        readonly: false,

        // 增加 easyui-validatebox 控件的自定义扩展属性；表示 easyui-validatebox 初始化时默认需要加载的样式类名；
        // 该值将会被作为 html-class 属性在 easyui-validatebox 初始化完成后加载至 html 标签上。
        cls: null,

        // 增加 easyui-validatebox 控件的自定义扩展事件，表示输入框在值改变时所触发的事件。
        onChange: function (value) { }
    };

    var methods = $.fn.validatebox.extensions.methods = {

        // 增加 easyui-validatebox 控件的自定义扩展属性；
