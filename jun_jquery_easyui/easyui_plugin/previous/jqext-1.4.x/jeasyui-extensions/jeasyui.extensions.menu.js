...
(function ($, undefined) {
    ...
    function bindMenuItemEvent(target, item) {
        ...
        item.bind('click.menu', function () {
            ...
            if (!this.submenu) {
                //hideAll(target, $(target).hasClass('menu-inline'));
                // 注释掉上面一行代码，并添加下面 3 行代码，以实现当 menu-item 的属性 hideOnClick 为 false 的情况下，点击菜单项不自动隐藏菜单控件。
                if (this.hideOnClick) {
                    hideAll(target, $(target).hasClass('menu-inline'));
                }
    ...
    // 以上代码为增加了 easyui-menu 控件中关于 menu-item 的 hideOnClick 属性的 jquery.menu.js 源码。
    ...
    $.extend($.easyui, {
        ...
        //                  回调函数 handler 和回调函数 onclick 的签名都为 function(e, item, target)，其中：
    ...
    // 另，本插件增加 easyui-menu 控件中 menu-item 的如下自定义扩展属性:
