...
//2017-01-03 Kant tabs组件关闭tab时释放iframe占用内存
var frame=$('iframe',tab);if(frame.length>0){frame[0].contentWindow.document.write(''); frame[0].contentWindow.close(); frame.remove();
if(navigator.userAgent.match(/MSIE ([0-9]+)./)){CollectGarbage();} }
tab.panel("destroy");
...
return row[opts.textField].toLowerCase().indexOf(q.toLowerCase())>-1; // 2015-10-10 Kant 将从头位置匹配改为任意匹配
