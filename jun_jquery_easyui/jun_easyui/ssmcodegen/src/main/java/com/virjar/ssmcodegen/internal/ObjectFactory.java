...
import com.virjar.ssmcodegen.internal.db.FullyQualifiedTable;
import com.virjar.ssmcodegen.internal.db.IntrospectedColumn;
import com.virjar.ssmcodegen.internal.db.IntrospectedTable;
import com.virjar.ssmcodegen.internal.types.JavaTypeResolverDefaultImpl;
...
public class ObjectFactory {
    ...
    public static Class<?> internalClassForName(String type) throws ClassNotFoundException {
        ...
        if (clazz == null) {
            clazz = Class.forName(type, true, org.mybatis.generator.internal.ObjectFactory.class.getClassLoader());
        }
    ...
    public static URL getResource(String resource) {
        ...
        if (url == null) {
            url = org.mybatis.generator.internal.ObjectFactory.class.getClassLoader().getResource(resource);
        }

        return url;
    }

    public static <T> T createInternalObject(Class<T> clazz) {
        T answer;

        try {
            answer = clazz.newInstance();
        } catch (Exception e) {
            throw new RuntimeException(getString("RuntimeError.6", clazz.toString()), e);
