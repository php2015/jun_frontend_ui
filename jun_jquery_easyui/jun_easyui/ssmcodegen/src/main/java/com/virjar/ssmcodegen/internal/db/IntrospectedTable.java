/*
 * Copyright 2006 The Apache Software Foundation Licensed under the Apache License, Version 2.0 (the "License"); you may
...
public abstract class IntrospectedTable {
    ...
    protected IntrospectedColumn primaryKeyColumn;
    ...
    public IntrospectedTable(TargetRuntime targetRuntime) {
        ...
        baseColumns =Lists.newArrayList();
        attributes = Maps.newHashMap();
        internalAttributes = Maps.newHashMap();
    }
    ...
    public IntrospectedColumn getColumn(String columnName) {
        if (columnName == null) {
            ...
            if (primaryKeyColumn.isColumnNameDelimited()) {
                if (primaryKeyColumn.getActualColumnName().equals(columnName)) {
                    return primaryKeyColumn;
                }
            } else {
                if (primaryKeyColumn.getActualColumnName().equalsIgnoreCase(columnName)) {
                    return primaryKeyColumn;
                }
            }
    ...
    public boolean hasJDBCDateColumns() {
        if (primaryKeyColumn.isJDBCDateColumn()) {
            return true;
        }
    ...
    public List<IntrospectedColumn> getAllColumns() {
        List<IntrospectedColumn> answer = Lists.newArrayList();
        answer.add(primaryKeyColumn);
    ...
    /**
     * Calculates an SQL Map file name for the table. Typically the name is "XXXX_SqlMap.xml" where XXXX is the fully
     * qualified table name (delimited with underscores).
     *
    ...
    public boolean hasAnyColumns() {
        return primaryKeyColumn != null || baseColumns.size() > 0;
    }
    ...
    public void setPrimaryKeyColumn(String columnName) {
        if (!StringUtils.isEmpty(columnName)) {
            throw new FeatureUnsupportException("not support composite keys");
        }
        ...
        while (iter.hasNext()) {
            ...
            if (introspectedColumn.getActualColumnName().equals(columnName)) {
                primaryKeyColumn = introspectedColumn;
                iter.remove();
                break;
            }
        }
