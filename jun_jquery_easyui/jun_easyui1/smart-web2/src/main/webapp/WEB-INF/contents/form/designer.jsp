...
<script type="text/javascript" src="${pageContext.request.contextPath}/plugins/form/js/form.import.config.js"></script>
   ...
   <div class="row m-l-0 m-r-0 p-l-5 p-r-5">
      <form method="post" id="form-prop" class="form-horizontal">
	     <input type="hidden" name="id" value="${form.id }">
          ...
          <!--导入配置属性-->
		  <input type="hidden" name="formImportConfig" id="import-config-args">
		...
		<div class="col-sm-4 p-l-10 p-r-0">
		  ...
		  <div class="col-sm-4 p-l-10 text-right">
			  <div class="form-group p-t-6">
				  <button type="button" id="form-import-config-btn" class="btn btn-info btn-sm cnoj-open-blank" data-uri="form/import/config?formId=${id}" data-title="导入配置" data-width="850"><i class="fa fa-wrench" aria-hidden="true"></i> 导入配置</button>
			  </div>
		  </div>
	  </form>
   </div>
	            ...
	            if(tag =='checkboxs') {
	               ...
	               // console.log(plugin);
	                if(is_new) {
						...
						if('listctrl' == attr_arr_all['leipiplugins']) {
                            arr['orgtitle'] = arr['orgtitle'] +'`' + attr_arr_all['sum_row_name'];
                            arr['bindField'] = arr['bindField'] + '`' + attr_arr_all['bind_sum_row_field'];
                            arr['rowSumBindTable'] = attr_arr_all['row_sum_bind_table'];
                            arr['rowSumBindTableField'] = attr_arr_all['row_sum_bind_table_field'];
                        }
