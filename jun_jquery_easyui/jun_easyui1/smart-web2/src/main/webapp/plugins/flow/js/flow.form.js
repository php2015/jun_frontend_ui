...
(function($){
	$.fn.flowForm = function(options) {
		...
		function initFormData(output) {
            ...
            autoFillListctrlListener(setting.userId, setting.orgId);
            countRowListener();
		}
 ...
 * @param orgId 部门ID
 */
function autoFillListctrlListener(userId, orgId) {
	$(".cnoj-auto-fill-listctrl").each(function () {
		var $this = $(this);
		if($this.hasClass("cnoj-auto-fill-listctrl-listener")) {
			return false;
		}
		//判断是否表单数据ID是否为空，如果不为空，则不自动填充；
		var formDataId = $('#form-data-id').val();
		if(utils.isNotEmpty(formDataId)) {
			return false;
		}
		...
		if(utils.isNotEmpty(relateField)) {
			...
			for(var i=0; i<relateFields.length; i++) {
                $("select[name=" + relateFields[i] + ']').on('change', function () {
                	var values = new Array(relateFields.length);
                    for(var i=0; i<relateFields.length; i++) {
                    	values[i] = $("select[name=" + relateFields[i] + ']').val();
                    }
	...
	function _fillValue($element, userId, orgId, values) {
 ...
 * 行统计监听
 */
function countRowListener() {
    $(".cnoj-row-count").each(function () {
        var $this = $(this);
