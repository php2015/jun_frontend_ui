(function($) {
	...
	/*
	 * $.extend($.fn.datagrid.defaults.editors, { combotree: { init:
	 * function(container, options){ var editor = jQuery('<input
	 * type="text">').appendTo(container); editor.combotree(options); return
	 * editor; }, destroy: function(target){
	 * jQuery(target).combotree('destroy'); }, getValue: function(target){ var
	 * temp = jQuery(target).combotree('getValue'); var temp2 =
	 * jQuery(target).combotree('getText');
	 * alert("getValue=>"+temp+"===>"+temp2); return temp+","+temp2; },
	 * setValue: function(target, value){ //var temp =
	 * value.toString.split(","); alert("setValue=11>"+value); //
	 * jQuery(target).combotree('setValue', temp[0]); }, resize:
	 * function(target, width){ jQuery(target).combotree('resize', width); } }
	 * });
	 */
