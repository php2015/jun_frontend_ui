...
(function (window, $, undefined) {
    ...
    coreUtil.isBoolean = function (obj) {
        return coreUtil.type(obj) == "boolean";
    };

    //  判断对象是否是数值类型；
    coreUtil.isNumber = function (obj) {
        return coreUtil.type(obj) == "number";
    };

    //  测试对象是否是字符串（String）类型值。
    coreUtil.isString = function (obj) {
        return coreUtil.type(obj) == "string";
    };

    //  测试对象是否是日期（Date）类型值。
    coreUtil.isDate = function (obj) {
        return coreUtil.type(obj) == "date";
    };

    //  测试对象是否是正则表达式（RegExp）。
    coreUtil.isRegExp = function (obj) {
        return coreUtil.type(obj) == "regexp";
    };

    //  测试传入的参数是否是一个 javscript 对象；
    coreUtil.isObject = function (obj) {
        return coreUtil.type(obj) == "object";
    };

    //  测试对象是否是数组（Array）。
    coreUtil.isArray = core_isArray || $.isArray;
    ...
    coreUtil.isUndefined = function (obj) {
        return obj == undefined || typeof obj == "undefined";
    };

    //  判断对象是否为空(Null)值。
    coreUtil.isNull = function (obj) {
        return obj == null;
    };

    //  判断对象是否为 "未定义" 值(即 undefined)或空(Null)值。
    coreUtil.isNullOrUndefined = function (obj) {
        return coreUtil.isUndefined(obj) || coreUtil.isNull(obj);
    };

    //  测试对象不为 "未定义" 值(即 undefined)、空(Null)值、Boolean-False值、空字符串值或数字 0 中的任何一种。
    coreUtil.isPositive = function (obj) {
        return obj ? true : false;
    };

    //  判断对象是否为 "未定义" 值(即 undefined)、空(Null)值、Boolean-False值、空字符串值或数字 0 中的一种。
    coreUtil.isNegative = function (obj) {
        return obj ? false : true;
    };

    //  测试对象是否是 jQuery 对象。
    coreUtil.isJqueryObject = function (obj) {
        return obj != null
            && obj != undefined
    ...
    coreUtil.isEmptyJquery = coreUtil.isEmptyJqueryObject = function (obj) {
        return coreUtil.isJqueryObject(obj) && !obj.length;
    };

    //  定义一个空函数
    coreUtil.noop = coreUtil.isFunction($.noop)
        ? $.noop
        : function () { };
    ...
    coreUtil.parseJqueryObject = coreUtil.parseJquery = coreUtil.wrapJquery = function (obj) {
        return coreUtil.isJqueryObject(obj) ? obj : $(obj);
    };
    ...
    coreUtil.guid = function (format, length) {
        ...
        return length >= 0
            ? coreString.left(ret, length)
            : coreString.right(ret, Math.abs(length));
    };
    ...
    coreUtil.getRootPath = function () {
        ...
        if (pathname.indexOf("/") > -1) {
            ...
            for (var i = 0; i < coreUtil.rootDegree && i < temps.length; i++) {
                result += "/" + temps[i];
            }
        }
        return result;
    ...
    coreUtil.resolveClientUrl = coreUtil.resolveUrl = function (url) {
        url = String(url);
        if (coreString.isNullOrEmpty(url) || coreString.isUrl(url)) {
            return url;
        }
    ...
    coreUtil.shieldContxtMenuEvent = function (callback) {
        document.oncontextmenu = function (e) {
            e.preventDefault();
            if (callback && coreUtil.type(callback) == "function") {
    ...
    function _loadJs(url, callback) {
        var heads = document.getElementsByTagName("head"),
            ...
            isFunc = coreUtil.isFunction(callback);
    ...
    coreUtil.eachCall = function (obj, call, times) {
        if (!coreUtil.isFunction(call)) {
            return;
        }
        if (obj == undefined) {
            obj = coreNullable.Object;
        }
        ...
        for (var i = 1; i <= times; i++) {
            for (var key in obj) {
                coreUtil.eachCall.call(this, obj[key], call, times - 1);
            }
        }
    };
    ...
    //  取消由 coreUtil.delay / $.util.delay 方法设置的函数延迟调用等待；该函数定义如下参数：
    //      id_of_delay: 必须。要取消的延迟等待函数调用的 handle 值（该值由方法 coreUtil.delay / $.util.delay 返回）。
    //      undelayJust: 可选。Boolean 类型值，表示要取消的延迟函数是否为即时调用函数（即通过 coreUtil.delay / $.util.delay 设定该函数时未传入 millisec 值或 millisec 值为 0。）；该参数默认为 false。
    ...
    coreUtil.tryExec = function (code, error, final) {
        ...
        if (typeof opts.code == "string") {
            opts.code = coreString.toFunction(opts.code);
        }
        if (typeof opts.error == "string") {
            opts.error = coreString.toFunction(opts.error);
        }
        if (typeof opts.final == "string") {
            opts.final = coreString.toFunction(opts.final);
        }
        try {
    ...
    coreUtil.equalsCompare = function (item1, item2) {
        return item1 == item2;
    };
    ...
    coreUtil.numericCompare = function (a, b) {
        if (!coreUtil.isNumeric(a) && coreString.isNumeric(a)) {
            a = parseFloat(a, 10);
        }
        if (!coreUtil.isNumeric(b) && coreString.isNumeric(b)) {
            b = parseFloat(b, 10);
        }
        if (a > b) return 1;
        if (b < b) return -1;
    ...
    coreUtil.equals = function (item1, item2, compare) {
        ...
        if (!isFunc) {
            compare = coreUtil.equalsCompare;
        }
    ...
    coreUtil.compare = function (item1, item2, compare) {
        var isFunc = coreUtil.isFunction(compare);
        if (!isFunc) {
            compare = coreUtil.numericCompare;
        }
    ...
    coreUtil.filterProperties = function (obj, propertieNames, excluding) {
        ...
        for (var k in obj) {
            if (excluding
                ? coreArray.contains(propertieNames, k)
                : !coreArray.contains(propertieNames, k)) {
                ret[k] = obj[k];
            }
        }
    ...
    (function (prototypes) {
        $.each(prototypes, function (i, item) {
    ...
    //  判断传入的字符串是否为Null或者为空字符串或者全是空格。
    coreString.isNullOrWhiteSpace = function (str) {
        return coreString.isNullOrEmpty(str) || coreString.trim(String(str)) == "";
    };
    ...
    coreString.replaceAll = function (str, substr, replacement, ignoreCase) {
        ...
        var regexp = coreUtil.isRegExp(substr)
            ? substr :
            new RegExp(String(substr), ignoreCase ? "gm" : "igm");
    ...
    coreString.format = function (str, arg1, arg2, arg3, argn) {
        ...
        var isArray = coreUtil.likeArrayNotString(arg1),
            data = ((isArray && !coreUtil.isString(arg1)) || coreUtil.isObject(arg1))
                ? arg1
                : coreArray.range(arguments, 1);
        if (isArray) {
    ...
    coreString.trim = coreString.trim || coreUtil.trim;

    //  返回一个新字符串，该字符串通过在此实例中的字符左侧填充空格或指定字符来来达到指定的总长度，从而使这些字符右对齐。
    coreString.padLeft = function (str, len, paddingChar) {
        str = coreString.isNullOrEmpty(str) ? "" : String(str);
        paddingChar = (coreString.isNullOrEmpty(paddingChar) || !paddingChar.length)
            ? " "
            : paddingChar;
        len = coreUtil.isNumeric(len) ? len : str.length;
        if (str.length < len) {
            for (; str.length < len; str = paddingChar + str) {
            }
        }
        return str;
    };
    ...
    coreString.padRight = function (str, len, paddingChar) {
        ...
        len = coreUtil.isNumeric(len) ? len : str.length;
        if (str.length < len) {
            for (; str.length < len; str += paddingChar) {
            }
        }
        return str;
    };
    ...
    coreString.leftBytes = function (str, len) {
        ...
        var length = coreString.getByteLen(str),
            i = 0,
            bytelen = 0,
            cc = document.charset;
        if (!cc) {
    ...
    coreString.isIDCard = function (str) {
        ...
        if (!/^\d{17}(\d|x)$/i.test(sId)) {
            if (sId.length == 15) {
                return coreString.isIDCard15(sId);
            }
            return false;
        }
    ...
    coreString.isNumeric = function (str, flag) {
        if (coreString.isNullOrEmpty(str)) {
            return false;
        }
        //验证是否是数字
        if (isNaN(str)) {
    ...
    //  将字符串对象转换成 数值(Number)。
    coreString.toNumber = function (str) {
        return coreString.toFloat(str);
    };
    ...
    coreNumber.precision = function (num, precision) {
        ...
        if (coreUtil.isNumeric(precision)) {
            return coreNumber.round(num, precision);
        } else {
            var str = String(num),
                i = str.indexOf(".");
    ...
    coreNumber.toFileSize = function (num, str) {
        ...
        if (!coreArray.contains(["b", "kb", "mb", "gb", "auto"], str)) {
            str = "auto";
        }
        var ret = null,
            kb = 1024,
        ...
        function toB(size) {
            return String(coreNumber.round(size, 2)) + "B";
        };
        function toKB(size) {
            return String(coreNumber.round(size / kb, 2)) + "KB";
        };
        function toMB(size) {
            return String(coreNumber.round(size / mb, 2)) + "MB";
        };
        function toGB(size) {
            return String(coreNumber.round(size / gb, 2)) + "GB";
        };
        return ret;
    };
    ...
    coreArray.isNullOrEmpty = function (array) {
        return !coreArray.likeArray(array) || !array.length;
    };
    ...
    coreArray.copy = coreArray.copyFrom = function (target, source) {
        ...
        if (coreUtil.isNumeric(l)) {
            while (i < l) {
                core_push.call(target, source[i++]);
            };
        } else {
    ...
    coreArray.fn.copyFrom = function (source) {
        return coreArray.copy(this, source);
    };
    ...
    //  创建一个和当前数组对象相同的数组并返回
    coreArray.clone = function (source) {
        return coreArray.copy([], source);
    };
    ...
    coreArray.contains = function (array, item, compare) {
        return coreArray.some(array, function (val) {
            return coreUtil.equals(val, item, compare);
        });
    };
    ...
    coreArray.indexOf = coreArray.indexOf || function (array, item, startIndex, count, compare) {
        array = coreArray.likeArray(array) ? array : [];
        var result = -1;
        if (!array.length) {
            return result;
    ...
    coreArray.lastIndexOf = coreArray.lastIndexOf || function (array, item, startIndex, count, compare) {
        array = coreArray.likeArray(array) ? array : [];
        var result = -1;
        if (!array.length) {
            return result;
    ...
    coreArray.splitPage = function (array, pageIndex, pageSize, sortby) {
        array = coreArray.likeArray(array) ? array : [];
        if (!pageIndex || !coreUtil.isNumeric(pageIndex) || pageIndex < 0) {
            pageIndex = 0;
        }
        if (!pageSize || !coreUtil.isNumeric(pageSize) || pageSize < 1) {
            pageSize = 10;
        }
        ...
        var startIndex = pageIndex * pageSize,
            ...
            data = coreArray.range(array, startIndex, stopIndex),
    ...
    //  注意：该方法会改变现有的数组。
    coreArray.removeAt = function (array, index) {
        return coreArray.removeRange(array, index, 1);
    };
    ...
    coreUtil.merge = coreArray.merge = coreArray.merge || function (array, item1, item2, itemn) {
        if (!coreArray.likeArray(array)) {
            throw "传入的参数 array 必须是一个数组对象。";
        }
    ...
    coreUtil.unique = coreArray.unique = function (array, compare, item1, item2, itemn) {
        ...
        if (coreUtil.isFunction(compare)) {
            coreArray.removeAt(args, 1);
        }
    ...
    coreArray.filter = coreArray.filter || function (array, compare, thisArg) {
        array = coreArray.likeArray(array) ? array : [];
        if (!coreUtil.isFunction(compare)) {
            return array;
        }
    ...
    coreArray.map = coreArray.map || function (array, callback, thisArg) {
        array = coreArray.likeArray(array) ? array : [];
    ...
    coreArray.min = function (array, compare) {
        ...
        return coreArray.reduce(coreArray.range(array, 1),
    ...
    coreArray.take = function (array, count) {
        array = coreArray.likeArray(array) ? array : [];
        if (!coreUtil.isNumeric(count) || count < 1) {
            count = 1;
        }
        var temps = [];
        for (var i = 0; i < array.length; i++) {
    ...
    coreArray.skip = function (array, count) {
        array = coreArray.likeArray(array) ? array : [];
        if (!coreUtil.isNumeric(count) || count < 0) {
            count = 0;
        }
        var temps = [];
    ...
    coreArray.forEach = coreArray.forEach || function (array, callback, thisArg) {
        var isArray = coreArray.likeArray(array),
            temps = isArray ? array : [],
            i = 0,
            length = temps.length;
        if (temps.length == 0) {
        ...
        if (isArray) {
            for (; i < length; i++) {
                if (callback.call(thisArg, temps[i], i, temps) == false) {
                    break;
                }
            }
        } else {
    ...
    coreArray.reduce = coreArray.reduce || function (array, callback, initialValue) {
        if (!coreUtil.isFunction(callback)) {
            throw "传入的参数 callback 不是一个函数。";
        }
        array = coreArray.likeArray(array) ? array : [];
    ...
    coreArray.reduceRight = coreArray.reduceRight || function (array, callback, initialValue) {
        if (!coreUtil.isFunction(callback)) {
            throw "传入的参数 callback 不是一个函数。";
        }
        array = coreArray.likeArray(array) ? array : [];
    ...
    coreArray.every = coreArray.every || function (array, callback, thisArg) {
        array = coreArray.likeArray(array) ? array : [];
        if (array.length == 0) {
            return true;
        }
    ...
    coreArray.some = coreArray.some || function (array, callback, thisArg) {
        array = coreArray.likeArray(array) ? array : [];
        if (!coreUtil.isFunction(callback)) {
            throw "传入的参数 callback 不是一个函数。";
        }
    ...
    coreArray.left = function (array, length) {
        array = coreArray.likeArray(array) ? array : [];
        if (!length || !coreUtil.isNumeric(length) || length < 0) {
            return [];
        }
        if (length > array.length) {
            return coreArray.clone(array);
        }
    ...
    //coreArray.equals = function (array, anotherArray) {
    //    if (!coreArray.isArray(array) || !coreArray.isArray(anotherArray) || array.length != anotherArray.length) {
    //        return false;
    //    }
    //    var tmp1 = coreArray.distinct(coreArray.clone(array)),
    //        tmp2 = coreArray.distinct(coreArray.clone(anotherArray));
    //    return tmp1.length == tmp2.length && coreArray.unique(tmp1, tmp2).length == tmp1.length;
    //};
    ...
    coreDate.isDate = function (date) {
        return coreUtil.isDate(date);
    };

    //  判断指定的对象是否为一个日期(Date)对象或者是一个日期格式的字符串。
    coreDate.likeDate = function (date) {
        return coreDate.isDate(date) || coreString.isDate(date);
    };

    //  将 String 或 Number 类型值转换成 Date 类型值；
    //  返回值：该方法返回一个新创建的 Date 类型值；
    coreDate.toDate = function (obj) {
        return coreUtil.isDate(obj)
            ? obj
            : (coreUtil.isString(obj) ? coreString.toDate(obj) : new Date(obj));
    };

    //  判断指定的日期字符串是否是合法的长日期格式；
    //  该函数依赖于 coreString.isLongDate 函数。
    coreDate.isLongDate = function (date) {
        return coreString.isLongDate(date);
    };

    //  判断指定的日期字符串是否是合法的短日期格式；
    //  该函数依赖于 coreString.isShortDate 函数。
    coreDate.isShortDate = function (date) {
        return coreString.isShortDate(date);
    };
    ...
    coreDate.equals = function (date1, date2) {
        return coreDate.compare(date1, date2) == 0;
    };
    ...
    coreUtil.isDOM = function (obj, doc) {
        ...
        return obj.nodeName
            && obj.nodeType == 1
            && obj.ownerDocument == doc;
    };

    //  判断指定的对象是否为一个 HTML-DOM 对象或者 jQuery 对象；该函数定义如下参数：
    //      obj：    要判断的对象；
    //      doc：    该参数可选；表示 obj 所在页面的 document；如果不定义该参数，则默认使用当前页面的 document；
    //  返回值：如果 obj 是一个 HTML-DOM 或者 jQuery 对象且存在于指定的 document 中，则返回 true；否则返回 false。
    ...
    coreUtil.hasParentWindow = function (win) {
        ...
        try {
            ...
            ret = p
                && coreUtil.isWindow(p)
                && coreUtil.isObject(p.document) ? true : false;
        } catch (ex) {
        }
        return ret;
    };
    ...
    coreUtil.getTop = function () {
        var w = window;
        while (coreUtil.hasParentWindow(w) && w != w.parent) {
            w = w.parent;
        }
        return w;
    };
    ...
    coreUtil.getParent = function () {
        var w = window;
        if (coreUtil.hasParentWindow(w) && w != w.parent) {
            w = w.parent;
        }
        return w;
    };
    ...
    coreUtil.hasParentJquery = function (win) {
        ...
        try {
            ...
            ret = p
                && coreUtil.isWindow(p)
                && coreUtil.isObject(p.document)
                && coreUtil.isFunction(p.jQuery) ? true : false;
        } catch (ex) { }
        return ret;
    };
    coreUtil.getTopJquery = function () {
        ...
        var w = window;
        while (coreUtil.hasParentJquery(w) && w != w.parent) {
            w = w.parent;
        }
        return w.jQuery;
    };
