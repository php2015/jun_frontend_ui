...
(function ($) {
    ...
    function bindingEvents(target) {
        ...
        if (opts.novalidate || t.is(":disabled")) {
            return;
        }
    ...
    function initTip(target) {
        ...
        $(target).tooltip(
            $.extend(
                {},
                opts.tipOptions,
                { content: state.message, position: opts.tipPosition, deltaX: opts.deltaX }
            )
        ).tooltip("show");
        state.tip = true;
    };
    ...
    function validate(target) {
        ...
        function validingType(name, param) {
            var val = box.val();
            var expression = /([a-zA-Z_]+)(.*)/.exec(name);
            var rule = opts.rules[expression[1]];
            if (rule && val) {
                var params = param || opts.validParams || eval(expression[2]);
                if (!rule["validator"].call(target, val, params)) {
                    ...
                    setMsg(opts.invalidMessage || message);
                    if (state.validating) {
                        initTip(target);
                    }
                    return false;
            ...
            if (opts.validType) {
                if ($.isArray(opts.validType)) {
                    for (var i = 0; i < opts.validType.length; i++) {
                        if (!validingType(opts.validType[i])) {
                            return false;
                        }
                    }
                } else {
                    if (typeof opts.validType == "string") {
                        if (!validingType(opts.validType)) {
                            return false;
                        }
                    } else {
                        for (var name in opts.validType) {
                            var param = opts.validType[name];
                            if (!validingType(name, param)) {
                                return false;
                            }
                        }
                    }
    ...
    function setNovalidation(target, novalidate) {
        var opts = $.data(target, "validatebox").options;
    ...
    $.fn.validatebox.methods = {
        options: function (jq) {
            ...
            return jq.each(function () {
                setNovalidation(this, false);
            });
        }, disableValidation: function (jq) {
            return jq.each(function () {
                setNovalidation(this, true);
            });
        }
    };
        ...
        events: {
            ...
            remote: {
                validator: function (value, param) {
                    ...
                    data[param[1]] = value;
                    var result = $.ajax({
                        url: param[0], dataType: "json", data: data, async: false, cache: false, type: "post"
                    }).responseText;
                    return result == "true";
                },
