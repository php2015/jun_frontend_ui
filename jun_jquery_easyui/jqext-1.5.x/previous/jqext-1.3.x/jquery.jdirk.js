...
(function (window, $, undefined) {
    ...
    //};
    //coreArray.prototype.equals = function (array) { return coreArray.equals(this, array); };
    ...
    //  判断指定的对戏那个是否为一个 HTML-DOM 对象或者 jQuery 对象；该函数定义如下参数：
    //      obj：    要判断的对象；
    //      doc：    该参数可选；表示 obj 所在页面的 document；如果不定义该参数，则默认使用当前页面的 document；
    ...
    //      fn :一个签名为 function (win, e) 的函数，其中 win 表示所在的 IFRAME/PAGE 执行该参数时传入的 window 对象，e 表示最初触发该循环函数调用的事件对象。
    //  备注：只有当 IFRAME/PAGE 所示的页面中引入了 jQuery 库，fn 指定的函数才能被执行。
    coreUtil.pageNesting = function (fn) {
        if (!coreUtil.isFunction(fn)) {
            return;
        }
        var wins = [],
            method = function (win) {
                if (coreArray.contains(wins, win)) {
                    return;
                }
                fn.call(win, win);
                wins.push(win);
                ...
                if (jq) {
                    jq("frame,iframe").each(function () {
                        try {
                            if (this.contentWindow && this.contentWindow.document && this.contentWindow.document.all && this.contentWindow.jQuery && this.contentWindow.jQuery.fn) {
                                method(this.contentWindow);
                            }
                        } catch (ex) { }
                    });
                }
            ...
            float = function (win) {
                var pw = win.parent;
                try {
                    if (pw && win != pw && pw.document && pw.document.all && pw.jQuery && pw.jQuery.fn && pw.jQuery.fn) {
                        method(pw);
                    }
                } catch (ex) { }
            };
            ...
            coreUtil.pageNesting(function (win) {
                for (var name in types) {
                    ...
                    win.jQuery(win.document).unbind(eventName).bind(eventName, eventData, function (e) {
                        callback.call(win, win, e, eventName, eventData);
                    });
                }
            });
        }
