...
function loadNewsAndKb() {
    $.get('/mvc/Blog/NewsAndKb.aspx', '', function (data) {
        if ($("#blog-news-kb").length) {
            $("#blog-news-kb").html(data);
        }
    }, 'html');
}
...
function loadAdUnderGoogle() {
    $.get('/mvc/blog/AdUnderGoogle.aspx', '', function (data) {
        $('#ad_under_google').html(data).show();
    },'html');
}
    ...
    this.loadComments = function () {
        ...
        if (commentCount <= 0) {
            ...
            if ($("#cnblogs_post_body pre[class]").length) {
                try { SyntaxHighlighter.highlight(); } catch (e) { }
            } 
            return;
        }
        var pageIndex = 0;
